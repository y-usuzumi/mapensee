...
pretty (In f) = render f

instance Pretty Lit where
  render (Lit val) = show val

instance Pretty Add where
  render (Add a1 a2) = "(" ++ pretty a1 ++ "+" ++ pretty a2 ++ ")"

instance Pretty Mult where
  render (Mult m1 m2) = "(" ++ pretty m1 ++ "*" ++ pretty m2 ++ ")"

instance (Pretty f, Pretty g) => Pretty (f :+: g) where
  render (Inl l) = render l
  render (Inr r) = render r
